...
export async function runSync(direction: 'push' | 'pull' = 'push', projectPath: string = process.cwd()) {
...
async function pushToGist(token: string, gistId: string | null, gistIdPath: string, projectPath: string, spinner: any) {
    const agentFiles = ['AGENTS.md', 'CLAUDE.md', 'GEMINI.md', 'COPILOT.md', '.cursorrules', 'MEMORY.md', 'README.md'];
    const filesToUpload: Record<string, GistFile> = {};
...
async function pullFromGist(token: string, gistId: string, projectPath: string, spinner: any) {
